...
$searchPaths = array(
    'core'      => $basePath . 'classes/',
    'libraries' => $basePath . 'libraries/',
);

// Add all modules to search paths
foreach ( glob( $basePath . 'modules/*', GLOB_ONLYDIR ) as $dir )
{
    $searchPaths[basename( $dir )] = $dir;
}

// Check if only one module should be generated
if ( isset( $argv[1] ) &&
     isset( $searchPaths[$argv[1]] ) )
{
    $searchPaths = array(
        $argv[1] => $searchPaths[$argv[1]],
    );
}
...
class arbitTopoLogicalSorting
{
    ...
    */
    public function getSortedNodeList()
    {
        ...
        if ( count( $this->edges ) )
        {
            throw new Exception( 'Cycle found in graph. This is actually impossible and indicates a bug in this software.' );
        }
...
foreach ( $searchPaths as $name => $path )
{
    ...
    // Skip graph for external libraries
    if ( $name !== 'libraries' )
    {
