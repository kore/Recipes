 ...
 * @license http://www.gnu.org/licenses/gpl-3.0.txt GPL
 */
class arbitCouchDbUserFacade extends arbitCouchDbFacadeBase implements arbitUserFacade
{
    /**
     * Get array of users
     *
    ...
    {
        $users = phpillowManager::getView( 'user' );
        $result = $users->query( 'user' );
    ...
    public function getUserDataByLogin( $login )
    {
        try
        {
            $view = phpillowManager::getView( 'user' );
            $result = $view->query( 'user', array(
                'key' => $login,
            ) );
        }
    ...
    public function createUser( $name )
    {
        ...
        {
            throw new arbitFacadeUserExistsException( $name );
        }
